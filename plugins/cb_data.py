...
ADMIN = os.environ.get("ADMIN", "5152847809")
...
async def doc(bot, update):
    ...
    ms = await update.message.edit("```DOWNLOADING TO DB PLEASE WAIT...```")
    used_limit(update.from_user.id, file.file_size)
    c_time = time.time()
    total_used = used + int(file.file_size)
    used_limit(update.from_user.id, total_used)
    try:
        path = await bot.download_media(message=file, progress=progress_for_pyrogram, progress_args=("```DOWNLOADING TO DB PLEASE WAIT...```",  ms, c_time))
    ...
    if value < file.file_size:
        await ms.edit("```UPLOADING TO TELEGRAM...```")
        try:
...
async def aud(bot, update):
    ...
    if thumb:
        ...
        c_time = time.time()
        try:
            await bot.send_audio(update.message.chat.id, audio=file_path, caption=caption, thumb=ph_path, duration=duration, progress=progress_for_pyrogram, progress_args=("```UPLOADING TO TELEGRAM...```",  ms, c_time))
            await ms.delete()
            os.remove(file_path)
            os.remove(ph_path)
        except Exception as e:
        ...
        c_time = time.time()
        try:
            await bot.send_audio(update.message.chat.id, audio=file_path, caption=caption, duration=duration, progress=progress_for_pyrogram, progress_args=("```UPLOADING TO TELEGRAM...```",  ms, c_time))
            await ms.delete()
            os.remove(file_path)
        except Exception as e:
            await ms.edit(e)
